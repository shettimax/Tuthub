...
$query = "SELECT * FROM account WHERE label='$walletid'";
  ...
  if (mysqli_num_rows($result) > 0) 
  {
        ...
        {
      $email=$row['username']; //fetch colmn username  so2 echo l8r
      $datee=$row['regdate']; // registration date
      $paid=$row['paid']; //payment statz 1 or 0
      $course=$row['course']; //what course the user is on
      $adderr=$row['adder']; //what course the user is on
      $place=$row['state'];//fetch the state of user
      $pix=$row['passport'];//hoton user
...
<div class="panel-body">
    ...
    <?php if($place=$placee)//if user state doesnt match tutor state
                                                   { ?>
                                                    <center>
                                                      <font color="gray"><h3>AVAILABLE TUTORS IN <?php
echo strtoupper("$place");
?></h3></font>
                                                      <img src="static/uploads/<?php echo $tutpix;?>" width="220" height="220" alt="user" title="<?php echo $email;?>"/>
                                                      <br>
                                                      <?php echo $tutor;?>
                                                      <br>
                                                      <?php echo $mail;?>
